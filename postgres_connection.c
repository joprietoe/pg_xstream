...
Portal pgr_SPI_cursor_open_with_arg(char *sql, int64 v1, int64 v2) {
    PGR_DBG("Opening Portal");
    Portal SPIportal;

    Oid argtypes[] = {INT8OID,INT8OID,INT8OID,INT8OID};
    Datum values[] = {v1,v2,v1,v2};
    
    SPIportal = SPI_cursor_open_with_args(NULL, sql, 4, argtypes, values,NULL,true,CURSOR_OPT_CUSTOM_PLAN);
    if (SPIportal == NULL) {
        elog(ERROR, "SPI_cursor_open returns NULL");
    }
    return SPIportal;
